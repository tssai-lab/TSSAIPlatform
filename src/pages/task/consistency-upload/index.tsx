...
const ConsistencyUploadTrain: React.FC = () => {
  ...
  const startTraining = async () => {
    ...
    if (!isApproved) {
      message.warning('代码模型版本未审核通过，不能用于训练');
      return;
    }
  ...
  return (
    <PageContainer
      ...
      subTitle="先上传代码模型包 ZIP 与 data.zip，再发起固定 profile 训练"
    ...
    >
      <Alert
        ...
        description="请分别上传 consistency_test_code.zip 与 consistency_test_data.zip。代码模型包上传后默认为 PENDING，需管理员审核通过后才能创建训练任务。"
      />
      ...
      >
        {currentStep === 0 && (
          <ProCard title="步骤 1：上传代码模型包 ZIP">
            <Form.Item
              name="codeName"
              label="代码模型包名称"
              rules={[{ required: true }]}
            ...
            <Form.Item label="代码模型包 ZIP 文件" required>
              <Upload
                accept=".zip"
                maxCount={1}
            ...
            {codeUploading && (
              <Progress percent={codeProgress} style={{ marginBottom: 12 }} />
            )}
        ...
        {currentStep === 1 && (
          <ProCard title="步骤 2：上传数据 ZIP">
            ...
            {dataUploading && (
              <Progress percent={dataProgress} style={{ marginBottom: 12 }} />
            )}
            <Space>
