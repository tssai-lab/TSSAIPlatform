...
```bash
curl -o test.txt "http://127.0.0.1:8080/api/files/download?objectName=smoke/test.txt"
```
...
| 方法 | 路径 | 说明 | 参数 |
| --- | --- | --- | --- |
| `GET` | `/list` | 查询数据集列表，默认返回每个资产最新版本 | 可选 query: `type` |
...
| `POST` | `/folder` | 上传 CV 文件夹并自动打包 | multipart: `datasetName`, `version`, `type`, `remark`, `files`, `paths` |
...
- 训练任务状态当前由后端元数据维护，主要值：`running`、`success`、`failed`、`stopped`。
- `progress` 为展示字段：`success` 为 100，`running` 为 50，`failed/stopped` 为 0。
...
| `status` | `VARCHAR(32)` | not null, index | `UPLOADING`/`COMPLETED` |
...
- 删除模型业务版本 `/api/model/delete` 时，会尝试删除对应 MinIO 对象；MinIO 删除失败不会阻断数据库删除。
