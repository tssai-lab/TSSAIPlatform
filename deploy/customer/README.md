...
**当前提供只读预检、安装配置生成、镜像归档校验/压缩和锁定镜像清单；还不是可直接安装的完整交付包。** 最新交付方式为“基础环境联网安装 + 项目镜像导入”，不再要求整套环境完全离线。11 个项目镜像已完成逐层校验与压缩；联网安装步骤及新装复演尚未完成，不应交给甲方当作一键安装器。镜像制作、接收和导入衔接见 [PROJECT_IMAGES.md](PROJECT_IMAGES.md)。
...
项目包默认合计 **11 个镜像（Docker 5 个、项目 containerd 6 个）**。把 PostgreSQL/Redis/MinIO 也随包交付是沿用现有五服务部署的最小变动建议，不表示它们由项目自研。联网安装基础环境仍需版本锁、下载源和摘要校验；下载失败停止并明确提示，不静默改用最新版或改动现有服务。

`image-catalog.json` 目前仍是 **24 个镜像的完整运行依赖清单**，不是“全部必须导出的镜像列表”：`purpose=kubernetes` 的 13 项在线获取，其余 11 项纳入项目包。`image_bundle.py` 已落实此分工并校验归档内容；安装入口还须落实正确运行时与运行别名。
...
| `image-catalog.json` | 24 个固定摘要的完整依赖清单；其中项目包 11 个、联网基础组件 13 个，不是全部要打包 |
...
1. 11 个项目镜像已实际导出，四组归档保留校验和及来源/运行别名；仍需完整接收校验和新运行时导入复演。Ubuntu 由客户预装。基础环境先检查已装版本，缺项按明确步骤联网安装；13 个基础组件镜像按固定摘要联网预拉，不准备整套环境离线包。
