...
**当前提供只读预检、安装配置生成、完整性校验和锁定镜像清单；还不是可直接安装的完整交付包。** 最新交付方式为“基础环境联网安装 + 项目镜像导入”，不再要求整套环境完全离线。镜像实际导出、联网安装步骤及新装复演尚未完成，不应交给甲方当作一键安装器。
...
以下数量来自当前锁文件，是本批准备范围，不表示已导出或部署成功；每个用途只保留当前锁定版本。
...
`image-catalog.json` 目前仍是 **24 个镜像的完整运行依赖清单**，不是“全部必须导出的镜像列表”：`purpose=kubernetes` 的 13 项在线获取，其余 11 项纳入项目包。D2b 的导出/安装入口须落实此分工，当前尚未实现实际导出。
...
1. 从当前锁文件筛选 **11 个项目相关镜像实际导出**，包含校验和和运行别名；Ubuntu 由客户预装。基础环境先检查已装版本，缺项按明确步骤联网安装；13 个基础组件镜像按固定摘要联网预拉，不准备整套环境离线包。
