...
while IFS='|' read -r _source_ref project_ref expected_id expected_size; do
  ...
  [[ $actual_size == "$expected_size" ]] || die "platform image size differs from lock: $project_ref"
...
echo "PASS: four project-specific application images match the Main baseline IDs"
