...
import { Button, Card, Descriptions, Empty, message, Popconfirm, Space, Spin, Table, Tag } from 'antd';
import React, { useEffect, useState } from 'react';
import { history, useParams } from '@umijs/max';
import { deleteDataset, fetchDatasetDetail, getDownloadUrl } from '@/services/platform';

const DatasetDetail: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const [datasetInfo, setDatasetInfo] = useState<API.DatasetDetail | null>(null);
  ...
  useEffect(() => {
    ...
    fetchDatasetDetail(id, { skipErrorHandler: true })
      ...
      .catch((error: any) => {
        message.error(error?.info?.message || error?.message || '加载数据集详情失败');
        setDatasetInfo(null);
      })
  ...
  if (loading) {
    return (
      <PageContainer title="数据集详情" onBack={() => history.push('/dataset/list')}>
  ...
  return (
    <PageContainer
      ...
      extra={
        <Space>
          <Button onClick={() => handleDownload(datasetInfo.latestVersion?.storagePath)}>下载最新版本</Button>
          <Popconfirm title="确认删除该数据集？删除后无法恢复。" onConfirm={handleDelete}>
            <Button danger>删除数据集</Button>
          </Popconfirm>
          <Button onClick={() => history.push('/dataset/list')}>返回列表</Button>
        </Space>
      }
    >
      <Card title="基本信息" style={{ marginBottom: 16 }}>
        <Descriptions column={2}>
          ...
          <Descriptions.Item label="最近上传时间">{datasetInfo.uploadTime || '-'}</Descriptions.Item>
          <Descriptions.Item label="版本数量">{datasetInfo.versions.length}</Descriptions.Item>
