...
import { deleteDataset, fetchDatasetList as fetchDatasetListService, getDownloadUrl } from '@/services/platform';
...
const DatasetList: React.FC = () => {
  ...
  const columns: ProColumns<API.DatasetItem>[] = [
    ...
    { title: '版本号', dataIndex: 'version', key: 'version', hideInSearch: true },
    {
      title: '上传时间',
    ...
    { title: '文件数', dataIndex: 'fileCount', key: 'fileCount', hideInSearch: true },
    {
      title: '操作',
      key: 'action',
      hideInSearch: true,
      render: (_, record) => (
        <Space>
          <Button type="link" onClick={() => history.push(`/dataset/detail/${record.assetId || record.id}`)}>
            详情
          </Button>
          <Button type="link" onClick={() => handleDownload(record.storagePath)}>
            下载
          </Button>
          <Popconfirm title="确认删除该数据集？" onConfirm={() => handleDelete(record.assetId || record.id)}>
  ...
  return (
    <PageContainer
      ...
      extra={[
        <Button key="upload" type="primary" onClick={() => history.push('/dataset/upload')}>
          + 上传数据集
        </Button>,
      ]}
    >
