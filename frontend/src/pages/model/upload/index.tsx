...
import { modelUploadChunk, modelUploadComplete, modelUploadInit } from '@/services/platform';
...
const ModelUpload: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (id && fp) {
      setResumeHint('检测到未完成的上传会话，请重新选择同一个 zip 文件继续上传。');
    }
  }, []);
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      const fileFingerprint = buildModelFileFingerprint(file, values.modelName, values.version, values.type);
      ...
      const chunkSize = initData?.chunkSize && initData.chunkSize > 0 ? initData.chunkSize : CHUNK_FALLBACK;
      const totalChunks =
      ...
      for (let partIndex = 0; partIndex < totalChunks; partIndex += 1) {
        if (uploaded.has(partIndex)) {
          setUploadPercent(Math.min(100, Math.round((finishedParts / totalChunks) * 100)));
          continue;
        }
        const start = partIndex * chunkSize;
        const end = Math.min(start + chunkSize, file.size);
        await modelUploadChunk(uploadId, partIndex, file.slice(start, end), requestOpts);
        finishedParts += 1;
        setUploadPercent(Math.min(100, Math.round((finishedParts / totalChunks) * 100)));
      }
  ...
  return (
    <PageContainer title="上传模型" onBack={() => history.push('/model/list')}>
      ...
      <Form form={form} onFinish={handleSubmit} layout="vertical">
        ...
        >
          <Input.TextArea rows={4} placeholder="请输入备注" maxLength={200} showCount />
        </Form.Item>
        <Form.Item
          name="file"
          label="模型包"
        ...
        <Form.Item extra={`当前仅支持单个 zip 模型包，大小限制 ${UPLOAD_CONFIG.MODEL.MAX_SIZE / 1024 / 1024 / 1024}GB。`}>
          <Space>
            <Button onClick={() => history.push('/model/list')} disabled={uploading}>
              取消
            </Button>
            <Button danger type="default" disabled={uploading} onClick={clearResumeStorage}>
              清除续传记录
            </Button>
