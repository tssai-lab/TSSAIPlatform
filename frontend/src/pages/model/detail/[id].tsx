...
import { deleteModel, fetchModelDetail, getDownloadUrl } from '@/services/platform';
    ...
    <PageContainer
      ...
      extra={
        <Space>
          ...
          <Popconfirm title="确认删除该模型？删除后无法恢复。" onConfirm={handleDelete}>
    ...
    >
      <Card title="基本信息" style={{ marginBottom: 16 }}>
        <Descriptions column={2}>
          ...
          <Descriptions.Item label="大小">{modelInfo.size || '-'}</Descriptions.Item>
          ...
          <Descriptions.Item label="存储路径" span={2}>
            <span style={{ fontFamily: 'monospace' }}>{modelInfo.storagePath || '-'}</span>
          </Descriptions.Item>
        ...
        {modelInfo.codeContent ? (
          <>
            ...
            <Space style={{ marginTop: 12 }}>
              <Button type="primary" size="small" icon={<CodeOutlined />} onClick={() => setCodePreviewVisible(true)}>
                弹窗查看
              </Button>
          ...
          <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="当前模型包中没有可预览的代码文件" />
        )}
      </Card>
      ...
      {codePreviewVisible && modelInfo.codeContent && (
        <CodePreview
          ...
          fileName={modelInfo.codeFileName || modelInfo.codeFilePath || 'model-code'}
