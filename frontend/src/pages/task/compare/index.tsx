...
import { Button, Card, Checkbox, Divider, Input, message, Select, Space, Table, Tag } from 'antd';
import type { ColumnType } from 'antd/es/table';
import React, { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { history, useSearchParams } from '@umijs/max';
import * as echarts from 'echarts';
import { fetchTaskList, fetchTaskDetail, fetchMlflowMetricsBulk, listExperimentVersions } from '@/services/platform';
import { MOCK_TASKS, genMockTaskMetrics } from '@/constants/mockData';
...
type ComparableGroup = { slug: string; modelName: string; datasetName: string; tasks: TaskMetricsData[] };
...
function toRelativeImprovement(series: { step: number; value: number }[]) {
  ...
  if (Math.abs(base) < 1e-9) return sorted.map((p) => ({ step: p.step, value: 0 }));
  return sorted.map((p) => ({
    step: p.step,
...
function comparableGroupSlug(modelName: string, datasetName: string): string {
  return `${modelName}|||${datasetName}`.replace(/\s+/g, '_').replace(/[^\w\u4e00-\u9fa5_-]/g, '_');
}
...
const TaskCompare: React.FC = () => {
  ...
  const [selectedMetrics, setSelectedMetrics] = useState<string[]>(['train_loss', 'val_accuracy']);
  /** 相同模型提升曲线使用的指标 */
  const [sameModelMetric, setSameModelMetric] = useState<string>('val_accuracy'); // 指标 key，与 MLflow 一致
  ...
  const experimentIdFromUrl = useMemo(() => searchParams.get('experimentId') || '', [searchParams]);
  const [experimentIdInput, setExperimentIdInput] = useState<string>('');
  const [comparePoolIds, setComparePoolIds] = useState<string[]>(() => loadComparePool());
    ...
    const load = async () => {
      ...
      try {
        ...
        if (expId) {
          ...
          try {
            const vr: any = await listExperimentVersions(expId, { skipErrorHandler: true });
            const vers: any[] = vr?.data ?? [];
            let list: API.TaskItem[] = vers.length
            ...
            const missing = want.filter((id) => !list.some((t) => String(t.id) === String(id)));
            for (const id of missing) {
              list.unshift(placeholderTaskRow(id, list.find((t) => t.modelName && t.datasetName)));
            }
            setTaskList(list);
            return;
          } catch {
            let list: API.TaskItem[] = [
              experimentVersionToTaskRow({ id: `${expId}-v1`, experimentId: expId, versionNo: 1, name: `实验 ${expId} · 第 1 版`, status: 'success' }),
              experimentVersionToTaskRow({ id: `${expId}-v2`, experimentId: expId, versionNo: 2, name: `实验 ${expId} · 第 2 版`, status: 'success' }),
              experimentVersionToTaskRow({ id: `${expId}-v3`, experimentId: expId, versionNo: 3, name: `实验 ${expId} · 第 3 版`, status: 'success' }),
            ];
            const want = idsFromUrl;
            const missing = want.filter((id) => !list.some((t) => String(t.id) === String(id)));
            for (const id of missing) {
        ...
        const placeholders = idsFromUrl.map((id) => placeholderTaskRow(id, hint));
  ...
  useEffect(() => {
    ...
    const valid = idsFromUrl.filter((id) => taskList.some((t) => String(t.id) === String(id)));
    if (valid.length === 0) {
    ...
    lastAppliedIdsRef.current = key;
    if (valid.length >= 2) {
      const selectedTasks = taskList.filter((t) => valid.includes(String(t.id)));
      const demo = selectedTasks.map((t, i) => withTaskMeta(genMockTaskMetrics(String(t.id), t.name, i), t));
      setMetricsData(demo);
    ...
    try {
      ...
      if (withRunId.length === 0) {
        const base = selectedTasks.length
          ? selectedTasks
          : ids.map((id) => placeholderTaskRow(id, taskList.find((t) => t.modelName && t.datasetName)));
        const demo = base.map((t, i) => withTaskMeta(genMockTaskMetrics(String(t.id), t.name, i), t));
        setMetricsData(demo);
      ...
      if (withRunId.length < details.length) {
        message.info(`部分任务无 Run ID，已跳过；共 ${withRunId.length} 个任务拉取指标`);
      }
      ...
      for (const t of withRunId) {
        ...
        try {
          const metrics = await fetchMlflowMetricsBulk(t.runId!, MLFLOW_METRIC_KEYS as unknown as string[]);
          results.push({
            taskId: t.id,
            taskName: t.name,
          ...
          results.push(withTaskMeta(genMockTaskMetrics(t.id, t.name, idx), meta as API.TaskItem));
        }
        idx += 1;
      }
      ...
      const base =
        (selectedRowKeys as string[]).map((id) => placeholderTaskRow(id, taskList.find((t) => t.modelName && t.datasetName))) ||
        [];
      const demo = base.map((t, i) => withTaskMeta(genMockTaskMetrics(String(t.id), t.name, i), t));
      setMetricsData(demo);
    ...
    sameModelGroups.forEach((group) => {
      ...
      const seriesImp = list
        ...
        .map((t, i) => ({
          ...
          type: 'line' as const,
          smooth: true,
          data: toRelativeImprovement(t.metrics[metric]!).map((p) => [p.step, p.value]),
          itemStyle: { color: TASK_COLORS[i % TASK_COLORS.length] },
        }));
  ...
  const resultColumns: ColumnType<(typeof comparisonRows)[0]>[] = [
    {
      ...
      width: 72,
      render: (v: number) =>
        v === 1 ? <Tag color="gold">#{v}</Tag> : v === 2 ? <Tag color="default">#{v}</Tag> : `#${v}`,
    },
  ...
  const metricSelectOptions = useMemo((): string[] => {
    const up = availableMetrics.filter((k) => k.includes('accuracy') || k.includes('mAP'));
  ...
  useEffect(() => {
    if (metricSelectOptions.length && !metricSelectOptions.includes(sameModelMetric)) {
    ...
    >
      <Card title="按实验ID加载版本" style={{ marginBottom: 16 }}>
        <Space wrap>
          <Input
            ...
            style={{ width: 420 }}
          />
          <Button type="primary" onClick={handleLoadExperiment} loading={loading}>
            加载版本
          </Button>
          <Button
            onClick={() => {
              ...
              history.push(`/task/compare?experimentId=${encodeURIComponent(demo)}`);
            }}
          >
            一键演示
          </Button>
        ...
        <div style={{ marginTop: 8, color: '#8c8c8c', fontSize: 12 }}>
          说明：任务列表接口默认只返回每个实验的最新版本；想对比同一训练的不同版本，请在此输入 experimentId 加载版本历史。
        </div>
      </Card>
          ...
          <Space size={8}>
            ...
            <Button
              ...
              onClick={() => {
                ...
                const next = Array.from(new Set([...(selectedRowKeys as string[]).map(String), ...comparePoolIds]));
            ...
            <Button
              ...
              onClick={() => {
                ...
                history.push(`/task/compare?ids=${comparePoolIds.map(encodeURIComponent).join(',')}`);
              }}
      ...
      >
          ...
          <Space size={[8, 8]} wrap>
            {comparePoolIds.map((cid) => (
              <Tag
                ...
                onClose={(e) => {
                  e.preventDefault();
                  const next = saveComparePool(comparePoolIds.filter((x) => x !== cid));
                  setComparePoolIds(next);
                }}
              >
        ...
        <div style={{ color: '#8c8c8c', fontSize: 12 }}>
          对比池只保存版本/训练的 ID（本地存储）。进入对比页时会自动补全这些 ID 对应的行，并在后端不可用时回退为演示曲线。
        </div>
      </Card>
      <Card title="选择训练任务" style={{ marginBottom: 16 }}>
        ...
        <div style={{ marginTop: 16 }}>
          <Space wrap>
            <Button type="primary" onClick={loadCompareData} loading={metricsLoading} disabled={selectedRowKeys.length === 0}>
              加载对比数据
            </Button>
            <Button onClick={loadDemoData} disabled={selectedRowKeys.length === 0}>
              使用演示数据
            </Button>
            <span style={{ color: '#8c8c8c', fontSize: 12 }}>
              已选 {selectedRowKeys.length} 个任务。性能提升曲线仅对「同一模型 + 同一数据集」下 ≥2 条任务生效
            </span>
          </Space>
        </div>
      </Card>
      ...
      {metricsData.length > 0 && (
        <Card title="模型对比结果" style={{ marginBottom: 16 }} extra={<span style={{ color: '#8c8c8c', fontSize: 12 }}>按验证准确率终值排序</span>}>
          <Table
            rowKey="key"
      ...
      {metricsData.length > 0 && sameModelGroups.length === 0 && (
        <Card style={{ marginBottom: 16 }}>
          <div style={{ color: '#8c8c8c' }}>
            未形成「同一模型 + 同一数据集」且条数 ≥2 的分组。请选多条模型与数据集均相同的任务（演示：三条 YOLOv8 + COCO；或两条 YOLOv8 + CIFAR-10）。
          </div>
        </Card>
      )}
          ...
          extra={
            <Space>
              ...
              <Select
                ...
                options={metricSelectOptions.map((k) => ({ label: METRIC_LABELS[k] || k, value: k }))}
              />
            </Space>
          }
        >
          <div style={{ color: '#8c8c8c', fontSize: 12, marginBottom: 16 }}>
            下列分组要求「模型名称」与「数据集名称」均一致，且至少 2 条任务。左图为指标随 Step 变化；右图为相对该次训练起点的提升幅度（%）。
          </div>
            ...
            return (
              <div key={slug} style={{ marginBottom: 32 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <div>
                    <div style={{ marginBottom: 8, fontSize: 13 }}>{METRIC_LABELS[sameModelMetric] || sameModelMetric}（过程）</div>
                    <div
                      ref={(el) => {
                  ...
                  <div>
                    <div style={{ marginBottom: 8, fontSize: 13 }}>相对训练起点的提升率（%）</div>
                    <div
                      ref={(el) => {
      ...
      {metricsData.length > 0 && (
        <Card
          title="过程参数曲线（跨任务）"
          style={{ marginBottom: 16 }}
          extra={<span style={{ color: '#8c8c8c', fontSize: 12 }}>勾选指标，每个指标一张图</span>}
        >
          {availableMetrics.length > 0 ? (
            <Checkbox.Group
              value={selectedMetrics}
              onChange={(vals) => setSelectedMetrics(vals as string[])}
              options={availableMetrics.map((k) => ({ label: METRIC_LABELS[k] || k, value: k }))}
            />
          ) : (
      ...
      {metricsData.length > 0 && availableMetrics.length > 0 && selectedMetrics.length > 0 && (
        <Card title="过程曲线图">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            {selectedMetrics.map((metricKey) => {
              const hasData = metricsData.some((t) => t.metrics[metricKey]?.length > 0);
              if (!hasData) return null;
              return (
                <div key={metricKey}>
                  <div style={{ marginBottom: 8, fontWeight: 500 }}>{METRIC_LABELS[metricKey] || metricKey}</div>
                  <div
                    ref={(el) => {
      ...
      {metricsData.length > 0 && (
        <div style={{ marginTop: 16, color: '#8c8c8c', fontSize: 12 }}>
          提示：真实数据来自 MLflow；演示数据用于本地预览。「性能提升」分组依据为任务中的模型名与数据集名均一致（与仅用模型名区分）。
        </div>
      )}
    </PageContainer>
  );
