...
function buildDemoVersions(taskInfo: any): API.TrainingExperimentVersion[] {
  ...
  const modelVersionId = taskInfo?.modelVersionId || taskInfo?.modelId || 'model-ver-demo-001';
  const datasetVersionId = taskInfo?.datasetVersionId || taskInfo?.datasetId || 'dataset-ver-demo-001';
  const base: Omit<API.TrainingExperimentVersion, 'id' | 'versionNo'> = {
    ...
    hyperParams: taskInfo?.hyperParams || { epochs: 10, batch_size: 32, learning_rate: 0.001 },
  ...
  return [
    ...
    },
    {
      ...
      hyperParams: { ...(base.hyperParams || {}), epochs: 20, batch_size: 64, learning_rate: 0.0005 },
      remark: '调参版本（演示）',
    },
    {
      id: `${experimentId}-v3`,
      ...
      hyperParams: { ...(base.hyperParams || {}), epochs: 30, batch_size: 64, learning_rate: 0.0003 },
      remark: '进一步调参（演示）',
    },
  ];
}
...
const TaskDetail: React.FC = () => {
  ...
  const [versionModalMode, setVersionModalMode] = useState<'create' | 'remark'>('create');
  const [versionModalLoading, setVersionModalLoading] = useState(false);
  const [versionBase, setVersionBase] = useState<API.TrainingExperimentVersion | null>(null);
  ...
  useEffect(() => {
    if (!id || !isInFlightStatus(taskInfo?.status)) return;
  ...
  useEffect(() => {
    ...
    if (!experimentId) {
      ...
      const patched = { ...(taskInfo as any), experimentId: demoExperimentIdRef.current };
  ...
  const refreshVersions = async (experimentId: string) => {
    try {
      const res: any = await listExperimentVersions(experimentId, { skipErrorHandler: true });
      const list = res?.data ?? [];
      setVersions(list?.length ? list : buildDemoVersions(taskInfo));
    } catch {
  ...
  const submitVersionModal = async () => {
    const experimentId = (taskInfo as any)?.experimentId || versionBase?.experimentId;
    ...
    try {
      ...
      const hyperParams = values.hyperParams ? JSON.parse(values.hyperParams) : undefined;
  ...
  const compareVersionColumns: ColumnsType<API.TrainingExperimentVersion> = [
    ...
    { title: '备注', dataIndex: 'remark', width: 140, ellipsis: true, render: (v: any) => v || '-' },
    ...
    >
      <Card title="任务信息" style={{ marginBottom: 16 }}>
        <Descriptions column={2}>
          ...
          <Descriptions.Item label="实验ID">{(taskInfo as any).experimentId || '-'}</Descriptions.Item>
          <Descriptions.Item label="版本号">{(taskInfo as any).versionNo ?? '-'}</Descriptions.Item>
                      ...
                      items: [
                        {
                          ...
                          label: '加入对比池',
                          onClick: () => {
                            const next = saveComparePool([record.id, ...loadComparePool()]);
                        ...
                        { key: 'remark', label: '修改备注', onClick: () => openUpdateRemark(record) },
                      ],
                    }}
                  >
      ...
      <Modal
        title={versionModalMode === 'create' ? '迭代新版本（同一实验）' : '修改备注'}
      ...
      >
        ...
        <Form form={versionForm} layout="vertical">
          {versionModalMode === 'create' && (
            <>
              <Form.Item name="codeVersionId" label="代码版本" rules={[{ required: true, message: '请输入代码版本' }]}>
          ...
          <Form.Item name="remark" label="备注（可选）">
            <Input placeholder={versionModalMode === 'create' ? '例如：基于 v1 迭代' : '例如：修正备注描述'} />
          </Form.Item>
          {versionModalMode === 'create' && (
            <Form.Item
              name="hyperParams"
            ...
            >
              <Input.TextArea rows={10} placeholder='{"epochs": 10, "batch_size": 32, "learning_rate": 0.001}' />
            </Form.Item>
          )}
        </Form>
      </Modal>
      ...
      >
        <div style={{ color: '#8c8c8c', fontSize: 12, marginBottom: 12 }}>
          勾选本实验下至少 2 个版本，跳转到「模型性能对比」查看曲线与指标差异（各版本对应一条训练记录 id）。
        </div>
        ...
        <Space style={{ marginTop: 16 }}>
          <Button type="primary" onClick={handleCompareVersions} disabled={compareVersionKeys.length < 2}>
            对比选中版本
          </Button>
          <Button onClick={() => history.push('/task/compare')}>打开对比页（自选）</Button>
        </Space>
      </Card>
