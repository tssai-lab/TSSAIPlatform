...
function getMockCurrentUserByRole(role: string): API.CurrentUser {
  ...
  return { name: current.name, userid: current.userid, role: role as API.UserRole };
}
  ...
  const fetchUserInfo = async () => {
    try {
      ...
      if (isDev && typeof window !== 'undefined') {
        const mockRole = window.localStorage.getItem('mock_role') as API.UserRole | null;
        if (mockRole && ['super_admin', 'normal_admin', 'user'].includes(mockRole)) {
