...
export const MOCK_TASKS: API.TaskItem[] = [
  ...
  { id: '4', name: 'YOLOv8-二阶段训练', modelName: 'YOLOv8-目标检测', datasetName: 'COCO-2017-训练集', createTime: '2026-01-24 10:00:00', status: 'success', progress:100 },
  { id: '5', name: 'YOLOv8-强数据增强', modelName: 'YOLOv8-目标检测', datasetName: 'COCO-2017-训练集', createTime: '2026-01-23 08:00:00', status: 'success', progress: 100 },
  ...
  { id: '6', name: 'YOLOv8-CIFAR 基线', modelName: 'YOLOv8-目标检测', datasetName: 'CIFAR-10-图像分类', createTime: '2026-01-22 09:00:00', status: 'success', progress: 100 },
  { id: '7', name: 'YOLOv8-CIFAR 调参', modelName: 'YOLOv8-目标检测', datasetName: 'CIFAR-10-图像分类', createTime: '2026-01-21 09:00:00', status: 'success', progress: 100 },
];
...
export function getMockRelatedTasksForDetail(
  ...
  if (modelId && datasetId) {
    ...
    matched = MOCK_TASKS.filter(
      (t) => normalizeModelNameForMatch(t.modelName) === mn && t.datasetName.trim() === ds,
    );
  }
  return [...matched].sort((a, b) => (a.createTime < b.createTime ? 1 : a.createTime > b.createTime ? -1 : 0));
}
...
function genMockCurve(
  ...
  for (let i = 0; i <= steps; i++) {
    let t = i / steps;
    if (flattenEnd) t = Math.pow(t, 0.85); // 末尾趋缓
    const base = trend === 'down' ? start - (start - end) * t : start + (end - start) * t;
...
export function genMockTaskMetrics(
  ...
  taskName: string,
  seed = 0,
): { taskId: string; taskName: string; runId: string; metrics: Record<string, { step: number; value: number }[]> } {
  const steps = 50;
  ...
  return {
    ...
    metrics: {
      ...
      val_accuracy: genMockCurve(steps, 0.1 + seed * 0.02, 0.92 - seed * 0.02, 'up', 0.015),
